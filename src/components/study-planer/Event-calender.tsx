...
const STATUS_COLORS = {
  todo: {
    bg: "bg-purple-500/20 border-l-4 border-purple-500 text-purple-200 hover:bg-purple-500 hover:text-white",
    dot: "bg-purple-500",
    badge: "bg-purple-500/20 text-purple-300",
  },
  process: {
    bg: "bg-orange-500/20 border-l-4 border-orange-500 text-orange-200 hover:bg-orange-500 hover:text-white",
    dot: "bg-orange-500",
    badge: "bg-orange-500/20 text-orange-300",
  },
  done: {
    bg: "bg-green-500/20  border-l-4 border-green-500  text-green-200  hover:bg-green-500  hover:text-white",
    dot: "bg-green-500",
...
export default function CalendarApp() {
  ...
  const { data: activities = [] } = useQuery<Activity[]>({
    ...
    queryFn: async () => {
      const url = user
        ? `/api/activities?userId=${user.uid}`
        : "/api/activities";
      ...
      <div className="flex flex-col h-screen overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-white/[0.05] border border-white/10 p-1 rounded-xl">
              ...
              <button
                ...
                className="px-3 py-1 text-xs font-bold bg-white/10 hover:bg-white/20 rounded-lg text-gray-300 transition-all"
              >
                Today
              </button>
              <button
        ...
        {view === "week" && (
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            {/* Day labels */}
            <div
              className="grid sticky top-0 z-[5] bg-[#030712]/90 backdrop-blur-md border-b border-white/5"
            ...
            >
              ...
              {weekDates.map((d, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex flex-col items-center justify-center h-16 border-r border-white/5 ${isToday ? "bg-indigo-500/5" : ""}`}
                  >
                    <span
                      className={`text-[10px] font-bold uppercase tracking-wider ${isToday ? "text-indigo-400" : "text-gray-500"}`}
                    >
                      {DAYS[i]}
                    </span>
                    <div
                      className={`mt-1 w-8 h-8 flex items-center justify-center rounded-full text-sm font-bold transition-all ${isToday ? "bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/30" : "text-gray-300"}`}
                    >
                      {d.getDate()}
                    </div>
                  </div>
            ...
            >
              ...
              <div className="flex flex-col">
                {HOURS.map((h) => (
                  <div
                    key={h}
                    className="flex justify-end pr-3 pt-2 text-[10px] font-bold text-gray-600 uppercase"
                    style={{
                      height: 80,
              ...
              {weekDates.map((d, di) => {
                const dayEvents = filtered.filter((a) =>
                  isSameDay(new Date(a.start), d),
                );
                const isToday = isSameDay(d, today);
                return (
                  <div
                    key={di}
                    className={`relative border-r border-white/5 ${isToday ? "bg-indigo-500/[0.02]" : ""}`}
                    style={{ height: 80 * 24 }}
                  >
                    {/* Hour lines */}
                    {HOURS.map((h) => (
          ...
          <div className="flex items-center gap-6">
            ...
            {(["todo", "process", "done"] as const).map((s) => (
              <div key={s} className="flex items-center gap-1.5">
                <div
                  className={`w-2 h-2 rounded-full ${STATUS_COLORS[s].dot}`}
                />
    ...
    <div>
      <div className="grid grid-cols-7 mb-2">
        {DAYS.map((d) => (
          <div
            key={d}
            className="text-center text-[10px] font-bold uppercase tracking-wider text-gray-600 py-2"
          >
            {d}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className={`min-h-[100px] rounded-xl p-2 border transition-all ${isToday ? "border-indigo-500/40 bg-indigo-500/5" : "border-white/5 bg-white/[0.02] hover:bg-white/[0.04]"}`}
            >
              <div
                className={`w-7 h-7 flex items-center justify-center rounded-full text-sm font-bold mb-1 ${isToday ? "bg-gradient-to-br from-indigo-500 to-purple-600 text-white" : "text-gray-400"}`}
              >
                {d.getDate()}
              </div>
              <div className="flex flex-col gap-0.5">
                {dayEvents.slice(0, 3).map((ev) => {
                  ...
                  return (
                    <div
                      key={ev._id}
                      className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md truncate ${STATUS_COLORS[s]?.badge ?? STATUS_COLORS.todo.badge}`}
                    >
                      {ev.title}
                    </div>
                  );
